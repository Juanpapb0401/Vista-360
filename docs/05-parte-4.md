...
- **Un registro de auditoría de accesos, separado del dato transaccional.** Cada lectura y cada escritura sobre información de un estudiante queda registrada con: quién la hizo (el `sub` del token, y si fue un token de servicio, en representación de qué proceso o de qué profesional), qué recurso se consultó o modificó, cuándo (con reloj sincronizado), y el resultado (autorizado o denegado).
